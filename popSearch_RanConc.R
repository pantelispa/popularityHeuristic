...
randomAgents <-  c(10,50,100)
randomAgents <- t(replicate(55,randomAgents))
randomAgents <- as.vector(randomAgents)
...
costSpace <- t(replicate(length(ratioSpace[1,])*3,costSpace))
costSpace <- as.vector(costSpace)
# ratioSpace <- rbind(t(ratioSpace),t(ratioSpace),t(ratioSpace)) # expended space
ratioSpace <- rbind(t(ratioSpace),t(ratioSpace),t(ratioSpace),t(ratioSpace),t(ratioSpace))
ratioSpace <- rbind(ratioSpace,ratioSpace,ratioSpace)#  huge space

# parameterSpace <- cbind(t(ratioSpace),costSpace) # limited space.
parameterSpace <- cbind(ratioSpace, costSpace, randomAgents) # extended space.
...
runOneCondition <- function(cond, parameterSpace, optThresholds){
  ...
  for (iter in 1:bigLoop){
    ...
    objectiveData <- rnorm(n = noAlts,mean,ratioA)
    uncer2 <- ratioA
    ...
    noRandomAgents <- parameterSpace[cond,4][[1]]
    theRandomAgents <- sample(noAgents,parameterSpace[cond,4])
    ...
    for (agent in 1:noAgents){
      ...
      if (agent %in% 1:noRandomAgents){ 
      # if (agent == 1 || agent %in% theRandomAgents){   
...
resultsFinal <- foreach (cond = 1:noParSets, .combine=comb, .init=list(list(), list())) %dopar% {
  ...
  runOneCondition(cond, parameterSpace, optThresholds)
}
proc.time() - ptm
...
save(parameterSpace, results, resultsDetailed, file = "popSearch_RanConc.RData")
